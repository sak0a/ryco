// Adapted from pingdotgg/t3code de28fa1ff3ac3c3a96bebd9ec5ce23dd09f61af0.
// Copyright (c) 2026 T3 Tools Inc. MIT; see ../LICENSE.
    ...
    fn sample(&mut self, config: &CollectorConfig, request_id: Option<String>) -> SnapshotEvent {
        ...
        let mut processes = tracked
            ...
            .filter_map(|pid| {
                ...
                // Never collect argv: provider arguments may contain credentials or prompts.
                let command = process.name().to_string_lossy().into_owned();
...
mod tests {
    ...
    #[test]
    fn avoids_commands_and_linux_tasks() {
        let refresh_kind = process_refresh_kind();

        assert_eq!(refresh_kind.cmd(), sysinfo::UpdateKind::Never);
