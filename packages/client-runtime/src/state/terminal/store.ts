...
import { parseScopedThreadKey, scopedThreadKey } from "../../scoped.ts";
...
export interface TerminalStateStorage {
  readonly getItem: (name: string) => string | null | Promise<string | null>;
  readonly setItem: (name: string, value: string) => void | Promise<void>;
  readonly removeItem: (name: string) => void | Promise<void>;
}

export interface TerminalStoreObservability {
  readonly enabled: () => boolean;
  readonly now: () => number;
  readonly record: (label: string, event: TerminalEvent, durationMs: number) => void;
}

const NOOP_OBSERVABILITY: TerminalStoreObservability = {
  enabled: () => false,
  now: () => 0,
  record: () => undefined,
};

function createMemoryStorage(): TerminalStateStorage {
...
export function createTerminalStateStore(options?: {
  readonly storage?: TerminalStateStorage;
  readonly observability?: TerminalStoreObservability;
}) {
  const storage = options?.storage ?? createMemoryStorage();
  const observability = options?.observability ?? NOOP_OBSERVABILITY;
  return create<TerminalStateStoreState>()(
    persist(
      (set) => {
        ...
        return {
          ...
          recordTerminalEvent: (threadRef, event) => {
            const perfEnabled = observability.enabled();
            ...
            if (perfEnabled) {
              observability.record(
                "web.terminal.store.events.record",
                event,
                observability.now() - startedAt,
              );
            }
          },
          applyTerminalEvent: (threadRef, event) => {
            ...
            if (perfEnabled) {
              observability.record(
                "web.terminal.store.events.apply",
                event,
                observability.now() - startedAt,
              );
            }
