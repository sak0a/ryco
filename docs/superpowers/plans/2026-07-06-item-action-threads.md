...
- [ ] `worktree.ts`: add `ItemActionWorkspacePlan` union
...
- [ ] Contract decode tests; typecheck; commit.

## Task 2: Server read-only resolver

- [ ] `worktreeOperations.ts`: `resolveActionWorkspace(input)`:
...
- [ ] Register in `gitRpc.ts` following `git.resolvePullRequest`.
- [ ] Tests: each tree outcome with fake projectionWorktrees/gitWorkflow.
- [ ] Typecheck + server tests; commit.

## Task 3: Server bootstrap execution

- [ ] `createWorktreeForProject` gains `options?: { existingThreadId?:
  ThreadId }`:
  ...
  - create path: with `existingThreadId`, replace the `thread.create`
    ...
    existing thread id.
- [ ] `dispatchBootstrapTurnStart`: when `bootstrap.prepareWorkspace` is
...
- [ ] Tests: bootstrap turn.start with each plan kind against fixture
  repos (mirror existing bootstrap tests if present; else cover
  `createWorktreeForProject` with `existingThreadId` + resolver
  integration).
- [ ] Typecheck + server tests (with signing disabled); commit.

## Task 4: Web derivation + prompts

- [ ] `itemActions.ts`: `ItemAction = { id, kind:
...
- [ ] `itemActionPrompts.ts`: one builder per action kind, referencing the
  attached context; append the `git status` first-step instruction when
  the plan reuses an existing checkout (`reuse-worktree` /
  `local-main-checkout`).
- [ ] Unit tests for both; typecheck; commit.

## Task 5: Banner UI + detail views

- [ ] `NeedsAttentionBanner.tsx`: warning-tinted container above the tab
...
- [ ] Render in `PullRequestDetail` (between header and tabs),
...
- [ ] Browser test: banner renders per state, absent when no actions.
- [ ] Typecheck + tests; commit.

## Task 6: Click flow + draft + send wiring

- [ ] `composerDraftStore.ts`: `DraftSessionState.pendingWorkspace?:
...
- [ ] `useStartItemActionThread.ts`: build intent from the item →
...
- [ ] Draft plan line: in the composer area for drafts with
...
- [ ] Send: ChatView passes the draft's `pendingWorkspace` into
...
- [ ] Tests: draft-store passthrough; bootstrap-builder unit test; browser
  test click → draft opens with prompt + chip + plan line.
- [ ] Typecheck + tests; commit.

## Task 7: Gate

- [ ] `bun fmt && bun lint && bun typecheck && bun run test` (server with
