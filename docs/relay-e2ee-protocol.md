...
```text
L1  T_ADV + T_HANDSHAKE + T_KEEPALIVE_FLUSH_MARGIN  ≤ RPC_KEEPALIVE_INTERVAL
L2  T_ADV + T_HANDSHAKE                             ≤ T_HANDSHAKE_NODE
...
`Ping` (§4.4). The longest contiguous such window is `T_ADV` followed by `T_HANDSHAKE`. Because
...
`T_HANDSHAKE`. L2 keeps `T_HANDSHAKE_NODE` above that sum so the two endpoints cannot both be
conforming and disagree about which one failed. It also leaves the node the slack the client
cannot afford: only the client is bound by L1.
...
- **Timers.** `T_ADV` is cancelled when a hello is sent or a mode is locked. The client's
  `T_HANDSHAKE` starts at hello emit (row K1); the node's `T_HANDSHAKE_NODE` starts at
  ...
  fires; a cancelled or superseded timer is ignored. The two deadlines are separate constants
...
prune landing inside the `T_ADV + T_HANDSHAKE` window (and, on the node side, inside
...
- **F10 — Mode machine** (§4.4): one case per transition row N1–N17 and K1–K24 — input
  ...
  **Timer and keepalive cases (these discharge §3.2.2 L1 and L2, and the §8.9 deadline).** The
  corpus MUST additionally include:
...
14. **The client's handshake and close budgets are coupled to a pinned transport keepalive.**
    ...
    free-running keepalive constrains how long negotiation may take: §3.2.2 L1 fixes `T_ADV` and
    `T_HANDSHAKE` so the whole window plus `T_KEEPALIVE_FLUSH_MARGIN` fits inside one
    `RPC_KEEPALIVE_INTERVAL`. Two costs are carried deliberately. First, the handshake budget is
