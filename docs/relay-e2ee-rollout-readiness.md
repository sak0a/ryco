...
| Two-node zero-touch and lifecycle  | Blocked                            | No disposable live Hub and two test nodes were available. Login-only discovery, local/account trust labels, independent upgrade, revocation, expiry, rotation, account switch, offline read-only, node restart, Hub reconnect, and old-node handling remain required. |
