  ...
  return (
    <AutocompletePrimitive.Portal>
      ...
      >
        <span
          className={cn(
            "app-surface relative flex max-h-full min-w-(--anchor-width) max-w-(--available-width) origin-(--transform-origin) rounded-lg border not-dark:bg-clip-padding shadow-lg/5 transition-[scale,opacity,translate] duration-(--app-motion-duration-pop) ease-(--app-motion-spring-gentle) will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            className,
          )}
        >
