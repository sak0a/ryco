...
#import "RycoMarkdownTextShadowNode.h"
#import "RycoMarkdownTextComponentDescriptor.h"
...
static void RycoMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<RycoMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void RycoMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<RycoMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = RycoMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        RycoMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol RycoMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface RycoMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<RycoMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<RycoMarkdownOutsideTapTarget>)target;

@end

static const void *RycoMarkdownOutsideTapCoordinatorKey =
    &RycoMarkdownOutsideTapCoordinatorKey;

@implementation RycoMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<RycoMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<RycoMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, RycoMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<RycoMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static RycoMarkdownOutsideTapCoordinator *
RycoMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  RycoMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, RycoMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[RycoMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        RycoMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface RycoMarkdownText () <RCTRycoMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface RycoMarkdownText () <RycoMarkdownOutsideTapTarget>
@end

@implementation RycoMarkdownText {
  UIView * _view;
  UITextView * _textView;
  RycoMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<RycoMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const RycoMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    RycoMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, RycoMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [RycoMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<RycoMarkdownTextProps const>(_props);
  ...
  RycoMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  RycoMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::RycoMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::RycoMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<RycoMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<RycoMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<RycoMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == RycoMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == RycoMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == RycoMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == RycoMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const RycoMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (RycoMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[RycoMarkdownTextRun class]]) {
      continue;
    }

    RycoMarkdownTextRun* textChild = (RycoMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::RycoMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::RycoMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> RycoMarkdownTextCls(void)
{
  return RycoMarkdownText.class;
}
