...
export function makeMobileE2eeAgreementKey(
  ...
  return {
    generate: () =>
      exclusive(async () => {
        // §14.5 is fail-closed and this is the first thing in the app that draws
        // key material, so this is where the preflight has to run: a refusal
        // means E2EE is unavailable on this device, never a weaker key.
        try {
          assertE2eeRuntimeGlobals();
        } catch {
          return agreementError("agreement_key_runtime_unavailable");
        }
