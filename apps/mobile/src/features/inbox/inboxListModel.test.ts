...
describe("mobile Inbox list", () => {
  ...
  it("pages settled rows by twenty in one flattened list", () => {
    ...
    const secondPage = flattenInboxSections({
      sections,
      settledOpen: true,
      settledVisibleCount: MOBILE_SETTLED_PAGE_SIZE * 2,
    });
    expect(secondPage.filter((item) => item.kind === "thread")).toHaveLength(42);
    expect(secondPage.at(-1)).toMatchObject({ kind: "show-more", remaining: 5 });
  });
});
