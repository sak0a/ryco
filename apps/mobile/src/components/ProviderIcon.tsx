...
// Provider brand marks. Every path is byte-identical to apps/web's Icons.tsx,
// and the driver-id mapping is copied from its `PROVIDER_ICON_BY_PROVIDER`.
//
// Before this, the component branched on "claudeAgent" and returned the OpenAI
// mark for EVERYTHING else — so Copilot, OpenCode and Cursor all rendered a
// competitor's logo. An unknown driver now gets a neutral glyph instead of a
// wrong brand.
//
// SIZING: three of these viewBoxes are not square. react-native-svg defaults to
// preserveAspectRatio="xMidYMid meet", so passing width=height on a non-square
// viewBox letterboxes the glyph and it reads visually smaller than its
// neighbours. Width is derived from the viewBox ratio; `size` is always the
// rendered HEIGHT, which is what keeps a row of mixed marks optically even.
...
export function ProviderIcon(props: ProviderIconProps) {
  ...
  // Monochrome marks follow the theme; branded ones keep their own colour.
  ...
  switch (props.provider) {
    ...
    case "copilot":
      // viewBox 256x208 — wider than tall.
      return (
        <Svg width={size * (256 / 208)} height={size} viewBox="0 0 256 208" fill="none">
          <Path
            fill={mono}
    ...
    case "cursor":
      // viewBox 466.73x532.09 — taller than wide.
      return (
        <Svg width={size * (466.73 / 532.09)} height={size} viewBox="0 0 466.73 532.09" fill="none">
          <Path
    ...
    case "opencode":
      // Two paths, drawn inner-square first. The frame punches its hole with
      // the DEFAULT nonzero fill rule — adding fillRule="evenodd" fills it in.
      // viewBox 32x40 — taller than wide.
      return (
    ...
    default:
      // A neutral rounded square, not a guessed brand. An unrecognised driver
      // should look unrecognised.
      return (
