...
import { type GrokSettings, type ModelSelection } from "@ryco/contracts";
...
export const makeGrokTextGeneration = Effect.fn("makeGrokTextGeneration")(function* (
  ...
  const generateIssueContent: TextGenerationShape["generateIssueContent"] = Effect.fn(
    "GrokTextGeneration.generateIssueContent",
  )(function* (input) {
    if (input.mode === "polish") {
      ...
      const decoded = yield* runGrokJson({
    ...
    const { prompt, outputSchema } = buildIssueContentTitlePrompt({ body: input.body });
    const decoded = yield* runGrokJson({
      operation: "generateIssueContent",
      cwd: input.cwd,
      prompt,
