...
// RYCO_CODEX_COLLAB_SCRIPT env var (a JSON file path) when the first turn
...
const script = JSON.parse(NodeFS.readFileSync(process.env.RYCO_CODEX_COLLAB_SCRIPT, "utf8"));
...
rl.on("line", (line) => {
  ...
  if (method === "turn/interrupt") {
    ...
    NodeFS.appendFileSync(
      `${process.env.RYCO_CODEX_COLLAB_SCRIPT}.interrupts`,
    ...
    if (
      ...
      write({
        ...
        error: {
          code: -32000,
          message: `expected active turn id ${script.expectedActiveTurnId} but received ${message.params?.turnId}`,
        },
      });
      return;
    }
