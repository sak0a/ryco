 ...
 * parent timeline at all. Three shipped bugs upstream came from that
 * decision being implicit (root thread registered as its own child; `error`
 * and `serverRequest/resolved` swallowed by a catch-all). These tests pin
 * the decision against a REAL capture rather than hand-written shapes.
 *
...
describe("codex multi-agent wire capture", () => {
  ...
  it("contains a /root self-activity emitted from a CHILD thread", () => {
    ...
    // (agentPath "/root"). Registering it makes the runtime intercept the
    // parent's own final message and turn/completed, so the thread hangs
...
describe("routeCodexChildNotification", () => {
  ...
  it("never routes child-owned thread lifecycle to the parent", () => {
    ...
    // suppressor) — the two must not drift.
    for (const method of [
      "thread/started",
      "thread/status/changed",
      "thread/archived",
  ...
  it("sends parent-owned and UNKNOWN methods to the parent path", () => {
    ...
    // swallowing it leaves approvals stuck. Unknown methods take the same
    // route by design — a codex update that adds a notification must
    // degrade to "parent sees it", never to silent loss.
