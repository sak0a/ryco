/**
 * GeminiAdapter - Gemini CLI ACP implementation of the provider adapter contract.
 *
...
import { applyGeminiAcpModelSelection, makeGeminiAcpRuntime } from "../acp/GeminiAcpSupport.ts";
import type { GeminiAdapterShape } from "../Services/GeminiAdapter.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";
import { GEMINI_BUILT_IN_MODELS } from "./GeminiProvider.ts";

const PROVIDER = ProviderDriverKind.make("gemini");
const GEMINI_RESUME_VERSION = 1 as const;
const ACP_PLAN_MODE_ALIASES = ["plan", "planning"];
const ACP_IMPLEMENT_MODE_ALIASES = ["code", "agent", "default", "chat", "implement"];
const ACP_APPROVAL_MODE_ALIASES = ["ask", "review"];
...
export interface GeminiAdapterLiveOptions {
  ...
  readonly resolveSettings?: Effect.Effect<GeminiSettings>;
}
...
function selectDecisionPermissionOption(
  request: EffectAcpSchema.RequestPermissionRequest,
...
export function makeGeminiAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const candidateModelsForSession = (model: string | undefined, settings: GeminiSettings) => [
      ...GEMINI_BUILT_IN_MODELS.map((entry) => entry.slug),
    ...
    const startSession: GeminiAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const effectiveGeminiSettings = options?.resolveSettings
          ...
          const acp = yield* makeGeminiAcpRuntime({
            geminiSettings: effectiveGeminiSettings,
            ...(options?.environment ? { environment: options.environment } : {}),
            childProcessSpawner,
            cwd,
            ...(resumeSessionId ? { resumeSessionId } : {}),
            clientInfo: { name: "ryco", version: "0.0.0" },
            candidateModels: candidateModelsForSession(
              geminiModelSelection?.model,
              effectiveGeminiSettings,
            ),
            ...acpNativeLoggers,
          }).pipe(
          ...
          const started = yield* Effect.gen(function* () {
            yield* acp.handleRequestPermission((params) =>
              Effect.gen(function* () {
                ...
                const optionId = selectDecisionPermissionOption(params, resolved);
                if (!optionId) {
                  return {
                    outcome: { outcome: "cancelled" as const },
                  };
    ...
    const respondToUserInput: GeminiAdapterShape["respondToUserInput"] = (
      ...
      Effect.fail(
        new ProviderAdapterRequestError({
          ...
          detail: `Gemini ACP user-input request '${requestId}' is not supported yet.`,
        }),
      );
