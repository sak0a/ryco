 ...
 * fallback.
 *
 * Provider catalogs live in `model-manifest.json`. The bundled copy ships
 * with every release; at runtime the service refreshes it from the same file
 * on the repository's `main` branch, so new Claude models can be published by
 * editing one file on GitHub without shipping a client update. Preference
 * order is remote, then the last successful on-disk copy, then the bundle. A
 * failed fetch never fails a provider check.
 *
 * Ported from pingdotgg/t3code (PRs #8227/#9084), adapted to Ryco's config,
 * settings, and schema surface.
 */
...
export const ModelManifestSchema = ModelManifestEnvelopeSchema.pipe(
  Schema.check(
