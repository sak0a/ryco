...
- Keep the Ryco server as the execution boundary.
...
Ryco already has a clean runtime boundary: the client talks to a Ryco server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Ryco server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Ryco server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Ryco runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Ryco server instance.
...
How does the client speak WebSocket to a Ryco server?
...
- best fit for hosted or self-managed remote Ryco deployments
...
For Ryco, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Ryco-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Ryco server
...
How does a Ryco server come to exist on the target machine?
...
The user or operator already runs Ryco on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Ryco:
...
The recommended Ryco flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Ryco availability.
3. Desktop launches or reuses a remote Ryco server.
...
This is the inverse of remote launch: a local Ryco server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Ryco server might be reached through direct `wss`.
...
Ryco already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Ryco should not copy that part.

Ryco already has the right runtime boundary:

- one Ryco server per environment
- ordinary HTTP/WebSocket between client and environment

So Ryco should borrow Zed's launch discipline, not its transport protocol.
